...
# Para la conversión del fichero de sonido capturado de Chainlist, en formato ogg a wav, que es el formato que acepta Azure Speech
#También es necesario instalar ffmpeg, que es una herramienta de línea de comandos para convertir archivos de audio y video a diferentes formatos.
# Además del pip install pydub, hay que añadir el path de ffmpeg a la variable de entorno PATH
...
def recommend2me(test_sessions_df):
    ...
    def save_results(prediction_df, item_mapping, session_mapping, save_path="./", cutoff=10, output_format="csv"):
        """
...
async def on_chat_start():
    ...
    # Envía un mensaje con la acción de grabación de audio
    #await cl.Message(
    #    content="Por favor, graba un mensaje de voz.",
    #    actions=[
    #        cl.Action(
    #            name="audio_recording",
    #            value="start",
    #            options={
    #                "audio.min_decibels": -50,
    #                "audio.initial_silence_timeout": 1000,
    #                "audio.silence_timeout": 2000,
    #                "audio.max_duration": 30000,
    #                "audio.chunk_duration": 5000,
...
async def call_tool(tool_call_id, name, arguments, message_history):
    ...
    if name == "get_fashion_recommendation":
        function_response = get_fashion_recommendation(
            yourQuery=arguments.get("yourQuery")
        )
        await synthesize_speech(arguments.get("yourQuery"))
...
async def call_gpt4(message_history):
    ...
    async for part in stream:
        try:
            ...
            if new_delta.content:
                ...
                #synthesize_speech(final_answer.content)
    ...
    if final_answer.content:
        #print("[call_gpt4].final_answer.content::",final_answer.content)
        # Sintetizar la respuesta generada por GPT-4
        await synthesize_speech(final_answer.content)
        # Una vez que se acaba de enviar todo el stream, liberar el botón de "Stop" y permitir la escritura de nuevo
        #await cl.Message(content="¡Puedes continuar preguntádome!").send()
...
async def sendSpeech2Chat(yourSpeech):
    message_history = cl.user_session.get("message_history")
    message_history.append({"role": "user", "content": yourSpeech})

    await call_gpt4(message_history)
   

# Función para sintetizar texto a voz, -text2speech-
async def synthesize_speech(text):
    ...
    #print("[synthesize_speech].Antes de sintetizar voz...")
    #result = speech_synthesizer.speak_text_async(text).get() 
    loop = asyncio.get_running_loop()
    with ThreadPoolExecutor() as pool:
        result = await loop.run_in_executor(pool, speech_synthesizer.speak_text_async(text).get)
    
    #print("[synthesize_speech].Tras de sintetizar voz...")
    

    #if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted: 
    #    print("Texto sintetizado en voz: [{}]".format(text)) 
    #else: 
    #    print("Síntesis de voz cancelada: {}".format(result.cancellation_details.reason)) 
...
# Función que se activa cuando termina la grabación de audio
# Define una función para manejar fragmentos de audio
...
async def on_audio_chunk(audio_chunk: cl.AudioChunk):
    try:
        # Añadir el fragmento de audio al buffer
        audio_buffer.write(audio_chunk.data)
        #print("Fragmento de audio recibido!")
    except Exception as e:
        print(f"Error procesando el fragmento de audio: {str(e)}")

@cl.on_audio_end
async def on_audio_end():
    try:
        # Reiniciar el puntero del buffer y leer el contenido
        audio_buffer.seek(0)
        ogg_data = audio_buffer.read()

        # Configurar ffmpeg si es necesario o configurar la variable de entorno PATH
        AudioSegment.ffmpeg = "C:\\Temp\\ffmpeg-7.0.2-essentials_build\\bin\\ffmpeg.exe"

        # Convertir el audio OGG a WAV
        ogg_audio = AudioSegment.from_ogg(BytesIO(ogg_data))
        wav_buffer = BytesIO()
        ogg_audio.export(wav_buffer, format="wav")
        wav_buffer.seek(0)

        # Guardar el archivo WAV como un fichero físico (requerido por Azure Speech)
        ...
        # Usar el archivo WAV como fuente de audio
        audio_config = speechsdk.audio.AudioConfig(filename=wav_filename)
        
        # Crear el recognizer para procesar el archivo de audio
        speech_recognizer = speechsdk.SpeechRecognizer(speech_config=speech_config, audio_config=audio_config)

        # Iniciar la transcripción (sin await)
        #result = await speech_recognizer.recognize_once_async()
        result = speech_recognizer.recognize_once_async().get()        
        print(f"Esto es lo que he reconocido:\n{result}")
        

        # Verificar si la transcripción fue exitosa
        if result.reason == speechsdk.ResultReason.RecognizedSpeech:
            
            await synthesize_speech(result.text)
            #print(f"Texto reconocido de tu speech: {result.text}")

            # Mostrar en la interfaz el texto transcrito
            await cl.Message(content=f"Esto es lo que quisiste decir? '{result.text}'").send()
            
            await sendSpeech2Chat(result.text)

        elif result.reason == speechsdk.ResultReason.NoMatch:
            adviseMessage = "Lo siento, no pude entender lo que decias!"
            await synthesize_speech(adviseMessage)
            await cl.Message(content=adviseMessage).send()
        else:
            await cl.Message(content=f"Hubo un error en el reconocimiento, {result.reason}").send()

        #Reiniciar el buffer de audio!
        set_audio_buffer(BytesIO())

    except Exception as e:
        # Manejo de errores
        print(f"Hubo un error procesando el audio, {str(e)}")
