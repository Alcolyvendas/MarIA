...
from Evaluation.metrics import precision, precision_recall_min_denominator, recall, MAP, MAP_MIN_DEN, \
...
def seconds_to_biggest_unit2(time_in_seconds, data_array = None):
...
def _create_empty_metrics_dict(cutoff_list, n_items, n_users, URM_train, URM_test, ignore_items, ignore_users,
                               diversity_similarity_object, get_failed_users=False):
    empty_dict = {}
...
def get_result_string_df(results_run_df, n_decimals=7):
    ...
    for cutoff in results_run_df.index:
        ...
        for metric in results_run_df.columns:
            if metric == 'FAILED_USERS':
                output_str += "{0}: {1}".format(metric, results_run_df.loc[cutoff, metric])
            else:
...
class Evaluator(object):
    ...
    def evaluateRecommender(self, recommender_object, get_failed_users=False, **rec_kwargs):
        """
        :param recommender_object: the trained recommender object, a BaseRecommender subclass
        :param URM_test_list: list of URMs to test the recommender against, or a single URM object
        :param cutoff_list: list of cutoffs to be use to report the scores, or a single cutoff
        :param get_failed_users: if true, return a list of failed users in results_df
        ...
        results_dict = self._run_evaluation_on_selected_users(recommender_object, self.users_to_evaluate,
                                                              get_failed_users=get_failed_users, **rec_kwargs)
    ...
    def _compute_metrics_on_recommendation_list(self, test_user_batch_array, recommended_items_batch_list, scores_batch,
                                                results_dict, get_failed_users=False):
        ...
        for batch_user_index in range(len(recommended_items_batch_list)):
            ...
            for cutoff in self.cutoff_list:
                ...
                if get_failed_users:
                    failed = test_user[not np.any(is_relevant, axis=0)]
                    results_current_cutoff['FAILED_USERS'].extend(failed)

        if time.time() - self._start_time_print > 300 or self._n_users_evaluated == len(self.users_to_evaluate):
            elapsed_time = time.time() - self._start_time
            new_time_value, new_time_unit = seconds_to_biggest_unit2(elapsed_time)
...
class EvaluatorHoldout(Evaluator):
    ...
    def _run_evaluation_on_selected_users(self, recommender_object, users_to_evaluate, block_size=None,
                                          get_failed_users=False, **rec_kwargs):
        ...
        results_dict = _create_empty_metrics_dict(self.cutoff_list,
                                                  ...
                                                  get_failed_users=get_failed_users)
